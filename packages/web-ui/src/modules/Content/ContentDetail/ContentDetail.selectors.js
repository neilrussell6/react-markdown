...
export const ContentDetailComponentSelector = createSelector(
  ...
  (selectedContent, selectedContentId) => {
    const markdown = isNil(selectedContent)
      ? null
      : propOr(null, 'markdown', selectedContent)
    const markdownContent = isNil(selectedContent)
      ? null
      : propOr(null, 'markdownContent', selectedContent)
    return {
      markdownContent,
      isLoading:
        !isNil(markdown) && !isNil(selectedContentId) && isNil(markdownContent),
    }
  },
)
